		...
		if ($offset === false || ($now - $offset < 5 && $now - $offset >= 0)) {
			...
			if ($btc_posts) {
				foreach ($btc_posts as $btc_post) {
					if (_btc_check_do_import($btc_post)) {
		...
		if (_btc_is_btc_comment($id_or_email)) {
			...
			switch ($id_or_email->comment_agent) {
				case 'btc_'.BTC_SRC_TWITTER:
					$default = 'http://spiurl.appspot.com/' . $id_or_email->comment_author;
					break;
 ...
 */
function _btc_check_do_import($btc_post) {
	// TODO: clean this up
	...
	if ($btc_post->initial_import_date_gmt == 0) {
		...
		if ($btc_post->hits > $btc_post->misses) {
			...
			return ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-600)) ? true : false; // every 10 minutes
		}
	} elseif ($btc_post->post_date_gmt > gmdate('Y-m-d H:i:s', time()-129600)) { // first 1.5 days
		if ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-600)) { // 10 minutes
			return true;
		...
		if ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-1800)) { // 30 minutes
			return true;
		} elseif ($btc_post->hits > $btc_post->misses) {
			return true;
		} else {
		...
		if ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-1800)) { // 30 minutes
			...
			return ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-21600)) ? true : false; // every 6 hours
		}
	} elseif ($btc_post->post_date_gmt > gmdate('Y-m-d H:i:s', time()-2592000)) { // first month
		if ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-3600)) { // 1 hour
			return true;
		} elseif ($btc_post->hits > $btc_post->misses) {
			return true;
		} else {
			return ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-43200)) ? true : false; // every 12 hours
		}
	} elseif ($btc_post->hits > $btc_post->misses) {
		return ($btc_post->last_import_date_gmt < gmdate('Y-m-d H:i:s', time()-1800)) ? true : false; // every 30 minutes
	}
	...
	do {
		...
		if (class_exists('SimpleXMLElement')) {
			if (!$xml || !empty($xml->errorCode) || empty($xml->comments)) {
				if (!empty($xml->errorCode) && $xml->errorCode == 'related-conversations-not-found') {
	...
	if ($entry['comment_src'] != '') {
		switch ($entry['comment_src']) {
			case BTC_SRC_BLOG:
				...
				$title = ($entry['post_title'] != '') ? '&#8220;' . $entry['post_title'] . '&#8221; (' . $entry['site_url'] . ')' : $entry['post_url'];
				break;
			case BTC_SRC_DIGG:
				$source = 'Digg';
				break;
