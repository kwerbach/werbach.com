...
if(isset($_POST["submit"]))						// THEM FORM HAS BEEN POSTED
{
	...
	$query 			= "SELECT ID, email, company, address1, address2, phone FROM " . $people_table . $where_clause . " LIMIT 0 , 1";
	...
	if ($result)								// QUERY WAS SUCCESSFUL
	{
		...
		if ($n_rows == 1)						// THERE IS ONE RECORD IN THE DB, COMPARE SOME FIELDS AND EMAIL IF THERE IS A CHANGE
		{
			...
			$phone		= $row["phone"];
			$body 		= compare_fields($company, $address1, $address2, $phone, $ID);
...
function compare_fields($c, $a1, $a2, $cp, $uid)
{
	global $not_people_fields;
	...
	if($cp <> $_POST["phone"])	// PHONE CHANGED
	{
		$body .= "Phone has changed from \"$phone\" to \"" . $_POST["phone"] . "\"\r\n<br/><br/>";
		$changed = 1;
	}		
	...
	if($changed == 1)		// THERE HAS BEEN A CHANGE
	{
		...
		$body .=  "http://www.werbach.com/db/people_edit_detail.php?key=" . $uid;
