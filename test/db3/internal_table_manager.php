...
if (!isset($HTTP_POST_VARS["show_all_fields"])){
	$show_all_fields = "";
} // end if
else{
	$show_all_fields = $HTTP_POST_VARS["show_all_fields"];
} // end else

// the position of the field the user wants to manage
if (!isset($HTTP_POST_VARS["field_position"])){
	$field_position = "";
} // end if
else{
	$field_position = $HTTP_POST_VARS["field_position"];
} // end else
...
if ($save == "1"){
	...
	for ($i=0; $i<count($fields_labels_ar); $i++){
		if (isset(${$int_fields_ar[1][1]."_".$i})){ // if isset the variable (it means that this field was in the form){
			...
			for ($j=1; $j<count($int_fields_ar); $j++){ // from 1 because the first is the name of the field ".${$int_fields_ar[$j][1]."_".$i};
				$sql .= $quote.$int_fields_ar[$j][1]."".$quote." = '".${$int_fields_ar[$j][1]."_".$i}."', ";
			} // end for
