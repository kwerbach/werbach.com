...
function write_to_csv()
{
	...
	$query = "SELECT people.* , supernova_2005.*, email_aliases.email_alias
				FROM people
				JOIN supernova_2005 ON people.ID = supernova_2005.people_id
	...
	if ($result)
		{
			...
			$dataToWrite = $dataToWrite . "\n\"$supernova_2005_id\",\"$people_id\",\"$last\",\"$first\",\"$email\",\"$email_alias\",\"$title\",\"$company\",\"$address1\",\"$address2\",\"$city\",\"$province\",\"$zip\",\"$country\",\"$phone\",\"$fax\",\"$cellphone\",\"$home_phone\",\"$website\",\"$seminar_event\",\"$showcase\",\"$priority_code\",\"$category\",\"$amount\",\"$payment_method\",\"$payment_received\",\"$email_format\",\"$status\",\"$date_registered\",\"$cc_trans_id\",\"$new_person\",\"$last_modified\"";
			}
