...
function text_field ($name="", $value="", $size=10, $maxlen="", $js="")
{
	$maxatt = empty($maxlen) ? "maxlength=\"$size\"" : "maxlength=\"$maxlen\""; // ENSURES MAXLEN
	$output = <<<EOQ
<input type="text" name="$name" value="$value" size="$size" $maxatt $js />\n
EOQ;
//$output = "$name";
	print $output;
//	return $output;
...
function db_select_list($query, $value_field, $label_field="", $match="")
{
	$result = safe_query($query);
	while (list($value_field,$label_field) = mysql_fetch_array($result))  // "list" assigns variables as if they were an array
	{
		if (empty($label_field)) { $label_field = $value_field; }
		$values[$value_field] = $label_field;		// populating an array called "values"
	}

	for ($i = 0; $i < count($values); $i++)
	{
		$option = each($values);		// "each" creates an array with a key and value an moves to the next item
		$selected = ($match == $option[key]) ? " selected" : "";
...
// MARKS AN OPTION AS SELCTED FOR A LIST
function am_selected($val,$match="|",$type)
{
	if($val == $match)
	{
		switch ($type)
