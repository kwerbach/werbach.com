  ...
  <tr>
    ...
    <td valign="top"> <font face="verdana,arial" size="2">
      ...
      <table width="400"  border="0" align="center" cellpadding="0" cellspacing="0">
        <?php
one_value("count(*)", "", "&#8226; People Registered: ");
one_value("sum(amount)", "", "&#8226; Total Revenue: $");
$payment_received = one_value("sum(amount)", "WHERE payment_received = 'yes'", "&#8226; Revenue Received: $");		// var use to calculate %ages
one_value("sum(amount)", "WHERE payment_received = 'no'", "&#8226; Revenue Outstanding: $");
?>
      </table>
      <br />
...
?>
      ...
      <br />
      <?php
write_detailed_sums_dec("cc_declined_reason", 0, 0);
?>
      <br>
      <br />
      <table width="400"  border="0" align="center" cellpadding="0" cellspacing="0">
...
function write_detailed_sums($field, $limit=0, $paid=2)
{
	...
	$query = "SELECT " . $field . ", SUM(amount) AS the_amount 
				FROM supernova_registrations 
				WHERE " . $field . " IS NOT NULL " . $not_paid_clause . "  AND seminar_year = '" . $reg_year . "' 
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$f = $row[$field];
		$a = $row["the_amount"];
		$p = ($a > 0) ? round($a / $payment_received * 100) : 0;
		$a = number_format($a, -2);
...
function write_detailed_sums_dec($field, $limit=0, $paid=2) // FOR DECLINED REASON
{
	global $payment_received;
	global $reg_year;
	if($paid == 0)		// NOT PAID
	...
	$query = "SELECT " . $field . ", count(" . $field . ") as num, SUM(amount) AS the_amount 
				FROM supernova_registrations 
				WHERE " . $field . " IS NOT NULL " . $not_paid_clause . "  AND seminar_year = '" . $reg_year . "' 
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$f = $row[$field];
		$a = $row["the_amount"];
		$n = $row["num"];		$p = ($a > 0) ? round($a / $payment_received * 100) : 0;
		$a = number_format($a, -2);
		$bar_width = round($p * 1.5);
		$sql_val = addslashes($f);
print <<<EOQ
		...
		<td class="form_label" align="right">$a</td>
		<td class="form_label" align="right">$p%</td>
		<td class="form_label" align="right" title="number of people">$n</td>
		<td width="34" align="center" class="form_label">
			<a href="attendees.php?where_clause=WHERE $field = '$sql_val' $not_paid_clause">view</a>
		</td>
		</tr>
