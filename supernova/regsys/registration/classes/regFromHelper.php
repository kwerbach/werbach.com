...
//		 	echo "don't have Code";		// TESTING
	...
	*/	
	...
	public function setTag($people_id, $tag)
	{
				...
				$sqlHelper	= new sqlHelper;
				$rsObj 		= $sqlHelper->queryCmd("SELECT NOW()");		// TESTING
...
//				$replacementArr = array();//"seminar_description"=>"YOUR PACKAGE", "reg_year"=>"2008");
				if (count($matches[0]) >= 1)
				{
					for($i = 0; $i <= count($matches); $i++)
					{
						...
						if(isset($matches[0][$i]))
						{
							...
							$subject = str_replace($matches[0][$i], strtoupper("<b>$replacementArr[$newStr]</b>"), $subject);
						}							
					}
				}
...
//				echo $sql;	// TESTING
				...
				while($rsRowArr = mysql_fetch_assoc($rsObj2))		// Put the quotes in an array
				{
					...
					echo "\t<div id=\"quoteText$cssSuffix\">" . $rsRowArr['quote_text'] . "</div>\n\t<div id=\"quoteSource$cssSuffix\">" . $rsRowArr['quote_source'] . "</div>\n";
					echo "</div>\n";
				}
			}
