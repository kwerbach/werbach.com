...
if ($overwrite = $HTTP_GET_VARS['overwrite'])
{
	$HTTP_POST_VARS = $HTTP_SESSION_VARS['post_vars'];
	$key = $HTTP_GET_VARS['key'];
	mysql_query("delete from people where ID=$key");		//remove old record (overwrite it)
}

if ($add = $HTTP_GET_VARS['add'])
{
	$HTTP_POST_VARS = $HTTP_SESSION_VARS['post_vars'];
}
...
$twitter = (get_magic_quotes_gpc()) ? $HTTP_POST_VARS['twitter'] : addslashes($HTTP_POST_VARS['twitter']);
...
if (!isset($HTTP_POST_VARS['marketer']))
{
 $marketer = "0";
}
...
if (!$overwrite) {
	...
	if (mysql_num_rows(mysql_query("select ID from people where email='$email'")) > 0)
	{
		print "<p><strong>The record you are trying to add appears to be a duplicate.</strong></p>";

		print "<p>Questionable entry:</p><blockquote>";
		...
		$duplicate_records = mysql_query("select ID,user_id,last,first,email,title,company,address1,address2,city,province,
		zip,phone from people where email='$email'");
		print "<p>Existing records with matching e-mail addresses (click on the record ID to edit):</p>";
		print "<blockquote>";
		while ($Row = mysql_fetch_assoc ($duplicate_records)) {
				
				print "<a href=\"people_edit_detail.php?key=" . $Row['ID'] . "\">" . $Row['user_id'] . "</a><br />";
				print $Row['first'] . " " . $Row['middle'] . " " . $Row['last'] . "<br />";
				print $Row['title'] . "<br />";
				...
				print $Row['city'] . ", " . $Row['province'] . "  " . $Row['zip'] . "<br />";
				print "<br />";
				print $Row['email'] . "<br />";
				print $Row['phone'] . "<br />";
				print "<br /><br />";
		...
		print "</blockquote>
			   <br /><p>Action to take:</p><ul><li><a href='people_add.php?overwrite=1&key=$key'>Overwrite</a> the existing record</li>
			   <li><a href=\"people_add.php?add=1\">Add</a> a new entry anyway</li>
			   <li>Leave the existing entry alone, <a href='people_add_form.php'>make no changes</a></li>
			   </ul></body></html>";

		$HTTP_SESSION_VARS['post_vars'] = $HTTP_POST_VARS;
		exit;
	}
}
}
...
$fieldname = $fieldname . "last,";		// FYI:  $X = $X . $Y   <==>   $X .= $Y
...
$insertfield = $insertfield . "'" . $userid . "',";
...
$insertfield = $insertfield . "'" . $twitter . "',";
...
$insertfield = $insertfield . "" . $marketer . ",";
$insertfield = $insertfield . "'" . $comments . "',";
$insertfield = $insertfield . "NOW(),";
...
{
?>
<p><div class='pagetext' align="center"><strong>Record successfully added!</strong></div>
<?
}
else
{
